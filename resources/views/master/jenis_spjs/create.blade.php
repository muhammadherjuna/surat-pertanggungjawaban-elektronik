...
@section('title', 'Tambah Jenis SPJ')

@section('content_header')
    <h1>Tambah Jenis SPJ</h1>
@stop

@section('content')
<div class="card card-primary">
    <div class="card-header">
        <h3 class="card-title">Form Tambah Jenis SPJ</h3>
    </div>
    <form action="{{ route('master.jenis-spjs.store') }}" method="POST">
        @csrf
        <div class="card-body">
            <div class="form-group">
                <label for="nama_jenis">Nama Jenis SPJ</label>
                <input type="text" name="nama_jenis" class="form-control @error('nama_jenis') is-invalid @enderror" id="nama_jenis" placeholder="Masukkan Nama Jenis SPJ" value="{{ old('nama_jenis') }}" required>
